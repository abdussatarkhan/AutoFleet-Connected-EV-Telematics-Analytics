...
# 📊 AutoFleet: Connected Electric Vehicle Fleet Telematics & Battery Health Analytics

**Connected Electric Vehicle Fleet Data Warehouse: Battery State of Health (SoH) degradation curves, kWh/100km energy efficiency, regenerative braking recovery, and smart charging.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/AutoFleet-Connected-EV-Telematics-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/AutoFleet-Connected-EV-Telematics-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/AutoFleet-Connected-EV-Telematics-Analytics)
...
**AutoFleet-Connected-EV-Telematics-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **Connected EV Fleet & IoT Telematics**.
...
### 1️⃣ Executive KPI Command Center (High-Tech Carbon & Electric Neon Blue (Dark))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_vehicles_ev  │──►│     fact_telemetry_trips │◄──┤     dim_battery_packs │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_charging_stations │   │     dim_routes_fleet    │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM autofleet_dw.fact_telemetry_trips f
JOIN autofleet_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/AutoFleet-Connected-EV-Telematics-Analytics.git
cd AutoFleet-Connected-EV-Telematics-Analytics
